...
// Kritische Fälle (z.B. left_to_pay > 1000€)
...
// Anzahl verarbeiteter Zahlungen in dieser Woche
...
// Letztes Import-/Verarbeitungsdatum
...
// TRANSACTION SUMMARY (Beispiel: Durchschnittliche Verzögerung in Tagen)
...
// CHART DATA (Monatliche Restzahlungen)
$chart_data = [];
for ($m = 1; $m <= 12; $m++) {
    $res = $conn->query("
...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .main h1{ font-family:'Space Grotesk',sans-serif; font-size:28px; font-weight:700; color:var(--red-dark); margin:0 0 18px; }

    .dashboard-grid{ display:grid; grid-template-columns: 1.6fr 1.6fr 0.9fr; grid-template-rows: auto auto; gap:16px; }
    ...
    .box{ background:#fff; border:1px solid var(--gray-light); border-radius:8px; box-shadow:0 1px 3px rgba(0,0,0,.06); display:flex; flex-direction:column; }

    .chart-title{ font-family:'Montserrat',sans-serif; font-weight:600; color:var(--red-dark); text-align:center; padding:10px 0 2px; font-size:16px; }
    .chart-title::after{ content:""; display:block; height:1px; background:var(--gray-light); width:58%; margin:6px auto 4px; }
    .chart-inner{ flex:1; padding:0 8px 10px; }
    #paymentsChart{ width:100% !important; height:210px !important; }

    .panel{ padding:14px 16px; height:100%; display:flex; flex-direction:column; }
    .panel h3{ margin:0 0 10px; font-size:16px; color:var(--red-dark); font-family:'Montserrat',sans-serif; font-weight:600; }

    ul.msgs{ margin:0; padding:0; list-style:none; }
    ul.msgs li{ display:flex; align-items:center; margin:8px 0; font-size:13px; padding:4px 0; }
    ul.msgs li .icon{ display:inline-flex; justify-content:center; align-items:center; width:22px; height:22px; border-radius:50%; margin-right:10px; font-size:16px; color:#fff; flex-shrink:0; }
    ul.msgs li.warn .icon{ background:orange; }
    ul.msgs li.success .icon{ background:#66BB6A; }
    ul.msgs li.info .icon{ background:#555; }

    .widgets-column{ height:100%; display:flex; flex-direction:column; gap:20px; padding-left:8px; }
    .widget{ height:110px; width:100%; max-width:260px; margin:0 auto; padding:14px 12px; text-align:center; border-left:4px solid var(--red-main); display:flex; flex-direction:column; justify-content:center; }
    ...
    footer{ position:fixed; bottom:0; left:0; right:0; background:var(--gray-light); color:#333; text-align:center; padding:10px; font-size:14px; border-top:1px solid #ccc; z-index:900; }
  </style>
</head>
<body>
  <div id="content">
      ...
      <section class="dashboard-grid">
        <div class="chart-cell">
          <div class="box chart-box">
            <div class="chart-title">Monatliche Zahlungssummen</div>
        ...
        <div class="summary-cell">
          <div class="box panel">
            ...
            <hr>
            <p>Durchschnittliche Verzögerung: <?= $avg_delay ?> Tage</p>
          </div>
        </div>
          ...
          <div class="box panel">
            ...
            <ul class="msgs">
              <li class="warn">
                ...
                <?= $unconfirmed ?> Zahlungen warten auf Bestätigung
              </li>
              <li class="success">
                <span class="icon material-icons-outlined">check_circle</span>
                <?= $processed_week ?> Transaktionen diese Woche verarbeitet
              </li>
              <li class="info">
                <span class="icon material-icons-outlined">info</span>
                Letzter Import: <?= $last_import_display ?>
              </li>
            </ul>
          </div>
        </div>
